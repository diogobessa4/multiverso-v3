...
supabase.auth.onAuthStateChange(async function(event, session) {
  ...
  try {
    if (session && session.user) {
      ...
      console.log('[Multiverso] autorizado:', autorizado);
    ...
    if (err && err.message === 'timeout_isAutorizado') {
  ...
  try {
    ...
    var rows = recentes.map(function(r) {
      ...
      return '<tr><td><span style="font-family:\'Orbitron\',monospace;color:var(--gold);font-size:.75rem">' + escHtml(r.codigo) + '</span></td>' +
             ...
             '<td>' + escHtml(String(r.quantidade)) + 'x</td>' +
             '<td style="font-size:.75rem;color:var(--muted)">' + data + '</td></tr>';
    }).join('');

    document.getElementById('tRecentes').innerHTML =
      '<thead><tr><th>Codigo</th><th>Produto</th><th>Cliente</th><th>Status</th><th>Qtd</th><th>Data</th></tr></thead>' +
      '<tbody>' + (rows || '<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--muted)">Nenhuma reserva ainda.</td></tr>') + '</tbody>';
